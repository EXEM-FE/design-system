...
function extractAndGroupCssVariables(filePath: string): Record<string, Record<string, string>> {
...
// 평면화된 토큰 매핑을 생성하는 함수
function generateTokens(variables: Record<string, string>, prefix: string): FlatTokens {
...
`;
    ...
    console.log(`✅ Generated: ${tokenName}.ts (${Object.keys(variables).length} tokens)`);
  }
